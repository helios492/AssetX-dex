...
export const getAllPools = async (api: ApiPromise) => {
  try {
    ...
    console.log("PoolGETError", error)
...
export const getAllLiquidityPoolsTokensMetadata = async (api: ApiPromise) => {
  ...
  if (pools) {
    ...
    const promises = poolsAssetTokenIds.map(async (item) => {
      if (item) {
    ...
    poolsTokenData.push(...results.filter(result => result !== null));
  }
  ...
  try {
    ...
    await Promise.all(
      pools.map(async (pool: any) => {
        ...
        let lpToken: LpTokenAsset | null = null;
